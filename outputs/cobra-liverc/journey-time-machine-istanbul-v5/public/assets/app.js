    ...
    <section class="journey-map-overlay" id="journeyMapOverlay" hidden aria-label="Distance raced road map">
      <div class="journey-map-shell">
        ...
        <div class="journey-map-controls">
          ...
          <button type="button" id="journeyPlay">▶ Play journey</button>
          <label class="journey-timeline">Journey date <strong id="journeyDateLabel">Latest</strong><input type="range" id="journeyDateSlider" min="0" max="0" value="0" step="1"></label>
        </div>
...
function openJourneyMap(driverKey) {
  ...
  journeyTimelineDates = [...new Set(Object.values(state.data.raceById).map(race => race.d).filter(date => date >= '2022-01-01'))].sort();
  const slider = $('journeyDateSlider');
