...
const state = { data: null, config: null, activeClass: '' };
...
function sameRank(a, b) {
  return a && a.total === b.total && a.highestDrop === b.highestDrop;
}
...
function championshipEvents(className) {
  ...
  return data.events.filter(event =>
    ...
    event.d >= config.startDate && event.d <= config.endDate &&
...
function render() {
  ...
  $('roundsHeld').textContent = `${events.length} / ${config.scheduledRounds}`;
...
async function init() {
  try {
    ...
    state.data = data;
    state.config = config;
    state.activeClass = config.classes[0];

    document.title = `${config.title} ${config.season} | COBRA`;
    $('championshipTitle').textContent = config.title;
    $('championshipSeason').textContent = `${config.season} championship standings`;
    $('championshipDates').textContent = `${dateFmt.format(new Date(`${config.startDate}T12:00:00Z`))} to ${dateFmt.format(new Date(`${config.endDate}T12:00:00Z`))}`;
    $('updatedStatus').textContent = `Last updated ${dateTimeFmt.format(new Date(data.meta.generatedAt))} UK time`;
    $('classTabs').innerHTML = config.classes.map((className, index) => `<button type="button" data-class="${escapeHtml(className)}" aria-pressed="${index === 0}">${escapeHtml(classLabels[className] || className)}</button>`).join('');
    $('rulesSummary').textContent = `100 points for 1st, reducing by one point per overall position. TQ earns 1 bonus point. Best ${config.bestRounds} of ${config.scheduledRounds} rounds count. DNS and DNF score their published overall position. Ties are decided by the highest dropped score; if that is also equal, the position remains tied.`;
