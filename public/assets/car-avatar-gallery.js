...
async function initialise() {
  ...
  try {
    ...
    for (const [hasImages, title] of [[true, 'Drivers with car avatars'], [false, 'Drivers without car avatars']]) {
      const group = drivers.filter(driver => Boolean(driver.images.length) === hasImages);
    ...
    status.textContent = drivers.length ? drivers.length + ' drivers · ' + withImages + ' with car avatars' : 'No drivers are available yet.';
